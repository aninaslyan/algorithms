export class HammingService {
  ...
  constructor() { }
   ...
   decode(input): string | void {
      ...
      let l = input.length;
      let i = 1;
      let output = this.pureDecode(input);
      let inputFixed = this.encode(output);
      ...
      if (sum) {
         output[sum - 1] === '1'
            ? output = this.replaceCharacterAt(output, sum - 1, '0')
            : output = this.replaceCharacterAt(output, sum - 1, '1');
      }
      return output;
   }
   ...
   check(input) {
      ...
      let inputFixed = this.encode(this.pureDecode(input));
   ...
   correct(input) {
      ...
      return this.encode(this.pureDecode(input));
   }

   replaceCharacterAt(str, index, character) {
   ...
   this.reCalc = function () {
      for (let key in this.responsibleFor) {
         if (this.responsibleFor.hasOwnProperty(key)) {
            ...
            if (Math.log(Number(key)) / Math.log(2) % 1 === 0) {
